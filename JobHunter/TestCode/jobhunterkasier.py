...
# Add a new job
insert_stmt = (
    "INSERT IGNORE INTO jobs (PostDate, Title, Location, Description, Apply_info) "
    "VALUES (%(created_at)s, %(title)s, %(location)s, %(description)s, %(how_to_apply)s)"
)
# Check if new job
check_stmt = """SELECT EXISTS(SELECT * FROM Jobs WHERE %(title)s)"""
# Delete given job
delete_stmt = """DELETE FROM Jobs WHERE Title = %(title)s"""
