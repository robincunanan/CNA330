...
def add_new_job(cursor, jobdetails):
    query = cursor.execute("INSERT INTO tablename(id, Type, url, Title, Description, Job_id, Created_at, Company, Location, How_to_apply" ") "
               "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);")
...
def check_if_job_exists(cursor, jobdetails):
    ## Add your code here
    query = cursor.execute("SELECT * FROM tablename WHERE Job_id =" + jobdetails['id'])
...
"""def check_if_job_exists(cursor, jobdetails):
    print("inside check if job exists")
    print("This is the ID" + jobdetails['id'])
    ## Add your code here
    query = cursor.execute("SELECT * FROM tablename WHERE id == " + jobdetails['id'])
